...
file_path="${work_path}/FASTQ_merged_Tumours"
...
##output directory
if [ ! -d "${work_path}/consensus_Tumours" ]; then
        mkdir -p "${work_path}/consensus_Tumours"
fi

if [ ! -d "${work_path}/consensus_output_Tumours" ]; then
        mkdir -p "${work_path}/consensus_output_Tumours"
fi
#s ${file_path}/*.bam  > ${work_path}/fq_files_test.txt
#create a list to save the bam files

##assign each sample into different ARRAYID
samples=${work_path}/fq1_TUM_files.txt
...
bam_path=${work_path}/consensus_Tumours/bamfiles
